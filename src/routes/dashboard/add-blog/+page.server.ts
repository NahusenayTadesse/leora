...
import { blogPost, projectCategory } from "$lib/server/db/schema";
...
export const actions: Actions = {
  addBlog: async ({ request, cookies, locals }) => {
    ...
    const { title, shortDescription, content, featured } = form.data;
    ...
    try {
      ...
      await db.insert(blogPost).values({
        title,
        slug: title?.at(0).toLowerCase() + title.slice(1).replace(/\s+/g, "-"),
        shortDescription,
        content,
        featured: imageName,
      });
