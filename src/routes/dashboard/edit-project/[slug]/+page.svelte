<script lang="ts">
	...
	$form.title = data?.singleProject?.title;
	$form.shortDescription = data?.singleProject?.shortDescription;
	$form.description = data?.singleProject?.description;
	$form.category = data?.singleProject?.category;
	$form.featured = data?.singleProject?.featured;
</script>

<svelte:head>
	<title>Edit {data?.singleProject?.title}</title>
</svelte:head>
...
<Card.Root class="flex w-full mt-4 flex-col gap-4 lg:w-xl">
	<Card.Header>
		<Card.Title class="text-2xl">Edit Project {data.slug}</Card.Title>
	</Card.Header>
	<Card.Content>
		<form
			use:enhance
